...
for user_id in group_people["g5"]:
    ...
    for case in group_exercises["g5"]:
        if int(case) in [2666,2664,2672,2456,2489,2494,2500,2522,2692,2176,2179,2061,2307,2438,2701]:
            continue
        if(classify_items[case]["题目类别"]=="数组"):
            user_scores[case] = 0
...
for user_id in group_people["g5"]:
    ...
    for j in range(len(cases)):
        ...
        if int(case_id) in [2666,2664,2672,2456,2489,2494,2500,2522,2692,2176,2179,2061,2307,2438,2701]:
            continue
...
df_group5.to_csv("./group5_results_array_0_1.csv",index=False,header=True)