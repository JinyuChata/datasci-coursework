...
for user_id in group_people["g2"]:
    ...
    for case in group_exercises["g2"]:
        if(case=="2701" or case =="2718"):
            continue

        if(classify_items[case]["题目类别"]=="查找算法"):
            user_scores[case] = 0
...
for user_id in group_people["g2"]:
    ...
    for j in range(len(cases)):
        ...
        if(case_id=="2804" or case_id=="2456" or case_id=="2309"):
            continue
        if(case_id=="2701" or case =="2718"):
            continue
        ...
        if len(uploads)>0 and type=="查找算法":
...
df_group2.to_csv("./group2_results_searching_0_1.csv",index=False,header=True)