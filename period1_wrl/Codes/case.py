...
# def entropyWeight(Datas):
# 	Datas = np.array(Datas)
# 	# 归一化
# 	P = Datas / Datas.sum(axis=0)

# 	# 计算熵值
# 	E = np.nansum(-P * np.log(P) / np.log(len(Datas)), axis=0)
...
#     inp = pd.read_csv(f"Datas\\TOPSIS\\{filename}.csv",encoding="utf-8")
...
# #将数据保存到csv
# Result.to_csv(f"Datas\\TOPSIS\\all-topsis.csv",encoding='utf-8-sig',index =True)

Result = pd.read_csv(f"data\\TOPSIS\\all-topsis.csv",encoding="utf-8")
# print(Result)
...
#绘制雷达图
data = Result.head(1)
data = data.append(Result.iloc[93])
data = data.append(Result.iloc[465])
data = data.append(Result.iloc[700])
