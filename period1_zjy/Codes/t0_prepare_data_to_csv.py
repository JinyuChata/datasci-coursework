...
test_data = pd.read_json("../Datas/test_data.json")
...
with open("../Datas/group-results.json", 'r', encoding='UTF-8') as f:
    user_group_info = json.load(f)

with open("../Datas/group-tests.json", 'r', encoding='UTF-8') as f:
    group_case_info = json.load(f)

with open("../Datas/cases_analysis_source.json", 'r', encoding='UTF-8') as f:
...
for group in group_converter:
    for typo in case_types:
        result_template[group_converter[group]][typo].to_csv(
            './Datas/' + group_converter[group] + '_' + p.get_pinyin(typo.encode('utf-8').decode(), '') + '.csv',
            index=False)
